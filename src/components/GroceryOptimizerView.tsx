...
export const GroceryOptimizerView: React.FC<GroceryOptimizerViewProps> = ({
  ...
  const handleConfirmLogExpense = (e: React.FormEvent) => {
    ...
    setLogSuccessMessage(isRo ? 'âœ… Bonul de ' + logAmount.toFixed(2) + ' ' + currency + ' a fost salvat si scazut din buget!' : 'âœ… Receipt for ' + logAmount.toFixed(2) + ' ' + currency + ' saved and deducted from budget!');
    ...
    <div className='space-y-6' style={{ fontFamily: 'Helvetica, Arial, -apple-system, sans-serif' }}>
      ...
      <div className='flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-stone-800 pb-5'>
        <div>
          <div className='flex items-center space-x-2 text-xs font-bold uppercase tracking-wider text-emerald-400'>
            ...
            <span>{isRo ? 'Optimizator Supermarket & Diversitate Culturala ðŸ‡²ðŸ‡¦ðŸ‡·ðŸ‡´' : 'Smart Supermarket & Cultural Grocery Optimizer ðŸ‡²ðŸ‡¦ðŸ‡·ðŸ‡´'}</span>
          </div>
          ...
          <p className='text-xs sm:text-sm text-stone-300 mt-1 max-w-2xl leading-relaxed'>
            {isRo ? 'Compara preturile reale intre Lidl, Kaufland, Carrefour, Mega Image, Penny si Auchan. Include selectie diversificata pentru Haytham (Maroc ðŸ‡²ðŸ‡¦) si Cati (Romania ðŸ‡·ðŸ‡´).' : 'Compare live store prices across Lidl, Kaufland, Carrefour, Mega Image, Penny & Auchan. Tailored for Haytham (Morocco ðŸ‡²ðŸ‡¦) & Cati (Romania ðŸ‡·ðŸ‡´).'}
          </p>
        </div>
        <div className='flex flex-wrap items-center gap-2'>
          <div className='flex items-center bg-stone-900 border border-stone-800 rounded-xl p-0.5'>
            <button type='button' onClick={() => setCurrentLang('ro')} className={'px-2.5 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ' + (isRo ? 'bg-emerald-500 text-stone-950' : 'text-stone-400 hover:text-white')}>ðŸ‡·ðŸ‡´ RO</button>
            <button type='button' onClick={() => setCurrentLang('en')} className={'px-2.5 py-1.5 rounded-lg text-xs font-bold transition cursor-pointer ' + (!isRo ? 'bg-emerald-500 text-stone-950' : 'text-stone-400 hover:text-white')}>ðŸ‡¬ðŸ‡§ EN</button>
          </div>
          {onOpenReceiptScanner && (
        ...
        <div className='space-y-6'>
          <div className='bg-stone-900 p-4 rounded-2xl border border-stone-800 flex flex-col sm:flex-row sm:items-center justify-between gap-3'>
            ...
            <div className='flex flex-wrap gap-1.5'>
              <button onClick={() => setQualityPref('CHEAPEST')} className={'px-3 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer ' + (qualityPref === 'CHEAPEST' ? 'bg-amber-500 text-stone-950 shadow-sm' : 'bg-stone-800 text-stone-400 hover:text-white')}>ðŸ¥‰ {isRo ? 'Pret Minim' : 'Cheapest'}</button>
              <button onClick={() => setQualityPref('BEST_VALUE')} className={'px-3 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer ' + (qualityPref === 'BEST_VALUE' ? 'bg-emerald-500 text-stone-950 shadow-sm' : 'bg-stone-800 text-stone-400 hover:text-white')}>ðŸ¥ˆ {isRo ? 'Calitate/Pret Optim' : 'Best Value'}</button>
              <button onClick={() => setQualityPref('PREMIUM')} className={'px-3 py-1.5 rounded-xl text-xs font-bold transition cursor-pointer ' + (qualityPref === 'PREMIUM' ? 'bg-cyan-500 text-stone-950 shadow-sm' : 'bg-stone-800 text-stone-400 hover:text-white')}>ðŸ¥‡ {isRo ? 'Bio & Premium' : 'Bio & Premium'}</button>
            </div>
          </div>
      ...
      {activeSubTab === 'budgetFitter' && (
        <div className='space-y-6 animate-in fade-in duration-200'>
          <div className='bg-gradient-to-br from-stone-900 via-stone-850 to-stone-900 p-6 sm:p-7 rounded-3xl border border-amber-500/30 shadow-2xl space-y-4'>
            <div className='flex items-center space-x-2 text-xs font-bold uppercase tracking-widest text-amber-400'><Coins className='w-4 h-4' /><span>{isRo ? 'Optimizare pe Banii Disponibili (Haytham ðŸ‡²ðŸ‡¦ & Cati ðŸ‡·ðŸ‡´)' : 'Budget-Fit Engine (Haytham ðŸ‡²ðŸ‡¦ & Cati ðŸ‡·ðŸ‡´)'}</span></div>
      ...
      {activeSubTab === 'bundles' && (
        <div className='space-y-4 animate-in fade-in duration-200'>
          <div className='bg-stone-900 p-5 sm:p-6 rounded-3xl border border-stone-800'><h3 className='text-base sm:text-lg font-bold text-white flex items-center gap-2'><Flame className='w-5 h-5 text-amber-400' /><span>{isRo ? 'Pachete Culturale Preconfigurate (Haytham ðŸ‡²ðŸ‡¦ & Cati ðŸ‡·ðŸ‡´)' : 'Preconfigured Cultural Meal Bundles (Haytham ðŸ‡²ðŸ‡¦ & Cati ðŸ‡·ðŸ‡´)'}</span></h3><p className='text-xs sm:text-sm text-stone-400 mt-1'>{isRo ? 'Adauga un set complet de ingrediente traditionale marocane si romanesti cu un singur click direct in lista ta.' : 'Add a full set of authentic Moroccan & Romanian staple ingredients with a single tap.'}</p></div>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
              ...
              <div key={bundle.id} className='p-5 sm:p-6 rounded-3xl bg-stone-900 border border-stone-800 hover:border-emerald-500/40 shadow-xl flex flex-col justify-between transition-all'>
                <div>
                  ...
                  <div className='mt-4 pt-3 border-t border-stone-800'><span className='text-xs font-bold text-stone-400 block mb-2'>{isRo ? 'Contine ' + bundle.items.length + ' produse esentiale:' : 'Contains ' + bundle.items.length + ' essential items:'}</span>
                    <ul className='space-y-2 text-xs text-stone-300'>{bundle.items.map((bItem) => { const cat = catalogMap.get(bItem.catalogId); return (<li key={bItem.catalogId} className='flex items-center justify-between'><span>â€¢ {cat?.name || bItem.catalogId}</span><span className='text-stone-400 font-mono font-bold'>x{bItem.quantity}</span></li>); })}</ul>
                  </div>
                </div>
          ...
          <div className='bg-stone-900 p-4 rounded-3xl border border-stone-800 space-y-3'>
            <div className='flex flex-col sm:flex-row gap-3'>
              ...
              <div className='flex gap-1.5 flex-wrap'>
                ...
                <button type='button' onClick={() => setCuisineFilter('MOROCCAN')} className={'px-3.5 py-2 rounded-xl text-xs font-bold cursor-pointer transition ' + (cuisineFilter === 'MOROCCAN' ? 'bg-amber-500 text-stone-950 font-black' : 'bg-stone-800 text-amber-400')}>ðŸ‡²ðŸ‡¦ {isRo ? 'Marocan' : 'Moroccan'}</button>
                <button type='button' onClick={() => setCuisineFilter('ROMANIAN')} className={'px-3.5 py-2 rounded-xl text-xs font-bold cursor-pointer transition ' + (cuisineFilter === 'ROMANIAN' ? 'bg-cyan-500 text-stone-950 font-black' : 'bg-stone-800 text-cyan-400')}>ðŸ‡·ðŸ‡´ {isRo ? 'Romanesc' : 'Romanian'}</button>
              </div>
            </div>
          </div>
      ...
      {inStoreMode && (
        <div className='fixed bottom-4 left-4 right-4 max-w-lg mx-auto z-40 bg-stone-900/95 backdrop-blur-md border border-emerald-500/50 p-4 rounded-3xl shadow-2xl flex items-center justify-between animate-in slide-in-from-bottom'>
          <div><span className='text-[10px] font-black uppercase text-emerald-400 tracking-wider block'>ðŸ›’ {isRo ? 'In Magazin â€¢ Progres: ' + checkedCount + ' / ' + shoppingList.length : 'In-Store Mode â€¢ ' + checkedCount + ' / ' + shoppingList.length}</span><div className='flex items-baseline gap-2'><span className='text-xl font-black text-white font-mono'>{inStoreRunningTotal.toFixed(2)} {currency}</span><span className='text-[11px] text-stone-400'>{isRo ? 'total bifat in cos' : 'total in cart'}</span></div></div>
